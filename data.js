...
heroes.classList.add("cards", "center");
body.appendChild(heroes);

// Вариант 1
dataHeroes.results.forEach(hero => {
    heroes.insertAdjacentHTML('beforeend', 
        `<div class="card">
            <img class="card__image-hero" src="${hero.image.url}" alt="${hero.name}">
            <h2 class="card__name-hero">${hero.name}</h2>
            <p class="card__description-hero">${hero.connections.relatives}</p>
        </div>`);
});


//Вариант с ассинхронным запросом немного не удался)
...
//     }
//     try{
//         let myData = getData(url);
//         myData.forEach(hero => {
//             heroes.insertAdjacentHTML('beforeend', 
//                 `<div class="card">
//                     <img class="card__image-hero" src="${hero.url}" alt="${hero.title}">
//                     <h2 class="card__name-hero">${hero.title}</h2>
//                     <p class="card__description-hero">${hero.thumbnailUrl}</p>
//                 </div>`);
//         });
//     }catch(error){
